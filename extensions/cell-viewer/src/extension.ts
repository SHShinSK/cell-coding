...
let lastLivePayload: string | undefined;
let activePanel: vscode.WebviewPanel | undefined;
let liveRunWatcher: fs.FSWatcher | undefined;
...
function liveRunPath(root: string): string {
  return path.join(root, 'viewer', 'live-run.json');
}

function viewerMediaDir(extensionUri: vscode.Uri): vscode.Uri {
...
async function regenerateTraces(root: string): Promise<void> {
...
function resolveCellRunner(root: string): {
  executable: string;
  baseArgs: string[];
  cwd: string;
  devMode: boolean;
  ...
  const cliPath = (cfg.get<string>('cliPath') || process.env.CELL_CLI || '').trim();
  const tsDir = path.join(root, 'typescript');
  ...
  const found = spawnSync(whichCmd, ['cell'], { encoding: 'utf-8', windowsHide: true });
  if (found.status === 0) {
...
/** cell run CLI로 live JSON 생성 */
interface RunCellCliOptions {
  outFile?: string;
  transpiled?: boolean;
...
async function runCellCli(
  root: string,
  cellFile: string,
  signalType: string,
  signalJson: string,
  opts: RunCellCliOptions = {},
): Promise<string> {
  ...
  if (opts.transpiled) args.push('--transpiled');
...
function viewerConfig(): {
  jaegerUiUrl?: string;
  runTranspiled: boolean;
  functionsPath?: string;
} {
  const cfg = vscode.workspace.getConfiguration('cellCoding');
  const jaeger = (cfg.get<string>('jaegerUiUrl') || process.env.JAEGER_UI_URL || '').trim();
  const fnPath = (cfg.get<string>('functionsPath') || '').trim();
  return {
    jaegerUiUrl: jaeger || undefined,
    runTranspiled: cfg.get<boolean>('runTranspiled') ?? false,
...
async function runInspectCli(
  ...
  cellFile: string,
  target?: string,
): Promise<{ ok: boolean; output: string; json?: string }> {
  const tsDir = path.join(root, 'typescript');
...
async function buildInspectPayload(root: string, cellFile: string): Promise<string> {
  const tsDir = path.join(root, 'typescript');
  const relCell = path.relative(tsDir, cellFile).replace(/\\/g, '/');
  const { stdout } = await execFileAsync(
    process.execPath,
    ['--import', 'tsx', 'viewer-inspect-cli.ts', relCell],
    { cwd: tsDir, windowsHide: true, maxBuffer: 10 * 1024 * 1024 },
  );
  return stdout;
}

/** cell test CLI (Cell Lab) 실행 */
async function runCellTestCli(
  root: string,
  cellFile: string,
  target?: string,
...
function loadViewerPayload(root: string): string | undefined {
  if (lastLivePayload) return lastLivePayload;
  const live = liveRunPath(root);
  if (fs.existsSync(live)) return fs.readFileSync(live, 'utf-8');
  const traces = tracesPath(root);
  if (fs.existsSync(traces)) return fs.readFileSync(traces, 'utf-8');
  return undefined;
}
...
function getReactWebviewHtml(
  ...
  html = html.replace(/(?:(href|src)=")(?!https?:|data:)([^"]+)"/g, (_match, attr: string, rel: string) => {
...
function wirePayloadHandshake(panel: vscode.WebviewPanel, payloadJson: string): void {
...
function watchLiveRunFile(root: string, panel: vscode.WebviewPanel): void {
...
function openViewerPanel(context: vscode.ExtensionContext, payloadJson: string): void {
  ...
  panel.webview.html = reactHtml ?? fallbackHtml(
    'React viewer bundle missing · React viewer 번들 없음',
  );

  wirePayloadHandshake(panel, payloadJson);
  const root = workspaceRoot();
  if (root) watchLiveRunFile(root, panel);
  panel.onDidDispose(() => {
...
export function activate(context: vscode.ExtensionContext): void {
  context.subscriptions.push(
    vscode.commands.registerCommand('cellCoding.refreshTraces', async () => {
      const root = workspaceRoot();
      if (!root) {
    ...
    vscode.commands.registerCommand('cellCoding.runActiveCell', async () => {
      ...
      try {
        await vscode.window.withProgress(
          { location: vscode.ProgressLocation.Notification, title: 'Cell run · 실행 중' },
          () =>
            runCellCli(root, cellFile, signalType, signalJson, {
              outFile: out,
    ...
    vscode.commands.registerCommand('cellCoding.inspectActiveCell', async () => {
      ...
      try {
        const { ok, output } = await vscode.window.withProgress(
          { location: vscode.ProgressLocation.Notification, title: 'Cell inspect · 분석 중' },
          () => runInspectCli(root, cellFile, target),
        );
    ...
    vscode.commands.registerCommand('cellCoding.testActiveCell', async () => {
      ...
      try {
        const { ok, output } = await vscode.window.withProgress(
          { location: vscode.ProgressLocation.Notification, title: 'Cell Lab · 테스트 실행' },
          () => runCellTestCli(root, cellFile, target),
        );
    ...
    vscode.commands.registerCommand('cellCoding.openViewer', async () => {
      ...
      if (!payload) {
        const act = await vscode.window.showWarningMessage(
          'No viewer traces found. Generate batch traces? · trace 없음. 일괄 생성할까요?',
          'Generate', 'Cancel',
        );
        if (act === 'Generate') {
